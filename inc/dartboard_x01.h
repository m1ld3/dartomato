...
public:
  ...
  void init_dartboard(uint32_t iStartVal) override;
...
private:
  ...
  void display_remaining(uint32_t iRemaining) const;
  void display_score(uint32_t iScore) const;
  void display_dart1(uint32_t iVal) const;
  void display_dart2(uint32_t iVal) const;
  void display_dart3(uint32_t iVal) const;
  void erase_all_darts();
  void erase_dart1() const;
  void erase_dart2() const;
  void erase_dart3() const;
  void set_focus_to_submit_button() const;
  void update_finishes(uint32_t iScore, uint32_t iNumberOfDarts) const;
  void submit_score_to_player(uint32_t iScore, uint32_t iNumberOfDarts, uint32_t iCheckoutAttempts, const QVector<QString> & iDarts) const;
  void handle_start_val(uint32_t iVal, const QChar & iType);
  void handle_score_in_range(uint32_t iVal, const QChar & iType);
  bool checkout_attempt_happened() const;
  void handle_score_equals_remaining(uint32_t iVal, QChar & iType);
  void handle_busted_score(const QChar & iType, bool iCheckoutAttempt);
  void handle_game_shot_score(uint32_t iVal, const QChar & iType);
  bool is_score_in_range(uint32_t iVal) const;
