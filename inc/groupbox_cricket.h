...
public:
  ...
  void increase_extra_points(ECricketSlots iSlot, uint32_t iPoints);
  void set_score();
  void update_extra_points_labels() const;
  void update_darts(QVector<QString> && iDarts);
  void handle_submit_button_clicked(uint32_t iNumberOfDarts, QVector<QString> & iDarts);
  void close_cricket_input() const;
...
private:
  ...
  void set_extra_points_label(ECricketSlots iSlot, uint32_t iPoints) const;
  void set_slot_label(ECricketSlots iSlot, uint32_t iHits) const;
  void display_leg_history() const;
  void perform_undo();
  ...
  void connect_slots();
  void filter_leg_scores_cutthroat(QVector<QVector<QString>> & oLegScores) const;
  void display_leg_scores(const QVector<QVector<QString>> & iLegScores) const;
  void display_leg_score_line(uint32_t iLegNumber, const QVector<QString> & iLegScore) const;
  static QString format_leg_score(const QVector<QString> & iLegScore);
  ...
  void handle_leg_won();
  void handle_switch_to_next_player() const;
  ...
  void submit_score_to_player(uint32_t iNumberOfDarts, const QVector<QString> & iDarts,
                              const std::array<uint32_t, static_cast<uint32_t>(ECricketSlots::SLOT_MAX)> & iExtraPointsCutThroat);
  bool has_leg_won() const;
