...
public:
  ...
  [[nodiscard]] bool is_slot_free(ECricketSlots iSlot, uint32_t iPlayer) const override;
  [[nodiscard]] bool is_score_bigger(uint32_t iScore) const override;
  [[nodiscard]] bool is_score_smaller(uint32_t iScore) const override;
  void increase_extra_points_of_other_players(ECricketSlots iSlot, uint32_t iPoints) override;
  QVector<uint32_t> compute_extra_points(ECricketSlots iSlot, uint32_t iPoints, uint32_t iPlayer) override;
